...
<div class="container mx-auto p-8">
    ...
    <!-- Form -->
    <form action="{{ route('superuser.jadwal.event.update', $jadwalEvent->id) }}" method="POST" class="bg-white p-8 rounded-2xl shadow-2xl max-w-2xl mx-auto">
        @csrf
        @method('PUT')
        <div class="space-y-6">
            <!-- Judul -->
            <div>
                <label for="judul" class="block text-indigo-900 font-semibold mb-2">
                    <i class="fas fa-heading text-indigo-500 mr-2"></i>Judul
                </label>
                <input type="text" name="judul" id="judul" value="{{ old('judul', $jadwalEvent->judul) }}"
                       class="w-full border border-gray-200 rounded-lg px-4 py-3 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition duration-200 {{ $errors->has('judul') ? 'border-red-500' : '' }}"
            ...
            <!-- Deskripsi -->
            <div>
                <label for="deskripsi" class="block text-indigo-900 font-semibold mb-2">
                    <i class="fas fa-align-left text-indigo-500 mr-2"></i>Deskripsi
                </label>
                <textarea name="deskripsi" id="deskripsi"
                          class="w-full border border-gray-200 rounded-lg px-4 py-3 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition duration-200 {{ $errors->has('deskripsi') ? 'border-red-500' : '' }}"
                          placeholder="Masukkan deskripsi event (opsional)" rows="5">{{ old('deskripsi', $jadwalEvent->deskripsi) }}</textarea>
                @error('deskripsi')
                    <p class="text-red-500 text-sm mt-2 flex items-center">
                        <i class="fas fa-exclamation-circle mr-2"></i>{{ $message }}
                    </p>
            ...
            <div>
                ...
                <input type="date" name="tanggal" id="tanggal" value="{{ old('tanggal', $jadwalEvent->tanggal) }}"
            ...
            <div>
                ...
                <input type="text" name="waktu" id="waktu" value="{{ old('waktu', $jadwalEvent->waktu) }}"
            ...
            <div>
                ...
                <input type="text" name="tempat" id="tempat" value="{{ old('tempat', $jadwalEvent->tempat) }}"
